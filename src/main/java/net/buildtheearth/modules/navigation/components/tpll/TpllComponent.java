...
public class TpllComponent extends ModuleComponent {
    ...
    public void addTpllToQueue(ByteArrayDataInput in, Player player) {
        ChatUtil.logDebug("Adding tpll event to queue... %s for %s" + in, player.getDisplayName());
        //Check the target server
        String targetServerName = in.readUTF();
        ChatUtil.logDebug("The name of the target server is: %s", targetServerName);
        if (targetServerName.equals(NetworkModule.getInstance().getBuildTeam().getServerName())) {
            ChatUtil.logDebug("The target server equals the current server");
            ...
            ChatUtil.logDebug("The coordinates of the tpll event are: %s %s", targetLatitude, targetLongitude);
            ...
            ChatUtil.logDebug("Created a bukkit location for this event");
            ...
            ChatUtil.logDebug("Successfully added the tpll event to the queue.");
        }
    }

    public void processQueueForPlayer(Player player) {
        ChatUtil.logDebug("Trying to process tpll queue for player: %s", player.getDisplayName());
        if(!tpllQueue.containsKey(player.getUniqueId())) return;
        Location tpllTarget = tpllQueue.get(player.getUniqueId());
        ChatUtil.logDebug("The tpll target is: %s", tpllTarget.toString());
        if (tpllTarget == null) return;

        if (tpllTarget.getWorld() == null) {
            player.sendMessage(ChatUtil.getErrorString("The %s world of this server is %s.", "earth", "unknown"));
            tpllQueue.remove(player.getUniqueId());
            return;
        }

        player.teleport(tpllTarget);
        ChatUtil.logDebug("Teleported the player to the tpll target.");
        tpllQueue.remove(player.getUniqueId());
        ChatUtil.logDebug("Successfully processed the tpll queue for this player.");
    }
    ...
    public void tpllPlayer(Player player, double[] coordinates, String targetServerName) {
        ChatUtil.logDebug("Starting universal tpll teleportation for %s to %s.", player.getDisplayName(), targetServerName);
        ...
        ChatUtil.logDebug("Teleported player to the target server.");
    }
